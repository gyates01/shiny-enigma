...
function StatTile({ label, value, sub, accent }) {
  return (
    ...
    >
      ...
      <div style={{ fontSize: 'var(--text-2xs)', color: 'var(--dim)', fontWeight: 500, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 'var(--space-2)' }}>{label}</div>
      <div style={{ fontSize: 'var(--text-2xl)', fontWeight: 700, color: 'var(--text)', lineHeight: 1, fontFamily: "'DM Mono', monospace" }}>{value ?? '—'}</div>
      {sub && <div style={{ fontSize: 'var(--text-sm)', color: 'var(--faint)', marginTop: 'var(--space-1)' }}>{sub}</div>}
    </div>
  )
}
...
function BarRow({ label, count, max, color }) {
  ...
  return (
    <div style={{ marginBottom: 10 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 'var(--text-md)', marginBottom: 'var(--space-1)' }}>
...
function BreakdownSection({ title, rows, labelKey, countKey, getRowColor }) {
  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontSize: 'var(--text-sm)', fontWeight: 600, color: 'var(--dim)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 14 }}>{title}</h2>
      {rows.map((row, i) => (
...
export default function Stats() {
  ...
  if (!stats) return <div className="page"><p className="dim" aria-live="polite">Loading...</p></div>
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20, maxWidth: 600, margin: '0 auto 20px' }}>
        <h1 style={{ fontSize: 'var(--text-xl)', fontWeight: 700 }}>Stats</h1>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(130px, 1fr))', gap: 10, marginBottom: 20, maxWidth: 600, margin: '0 auto 20px' }}>
        ...
        <StatTile label="Avg calories" value={stats.avg_calories ? `${stats.avg_calories}` : null}  sub="kcal" accent="var(--blue)" />
